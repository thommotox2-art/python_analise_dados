...
#vai buscar agora os cartões que possuem o  descritivo do site ou seja uma Card entity     
    ...
    for card in cards:
        try:
            ...
            # CORRIGIDO: A lógica de atribuição de "diretor" foi corrigida e simplificada.
            if diretor_tag:
                diretor = diretor_tag.text.strip()
                diretor = diretor.replace("Direção:", "").replace(",", "").replace("|", "").replace("\n"," ").replace("\r"," ").strip()
            else:
                diretor = "N/A"
            ...
            # CORRIGIDO: Capturar o ano de lançamento do filme em um novo local.
            ano_tag = filme_soup.find("span", class_="date")
